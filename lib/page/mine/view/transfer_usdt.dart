...
class TransferUsdtState extends State<TransferUsdt> {
  ...
  postData(isIfc) async {
    var params;
    if (isIfc) {
      params = {"from": "IFC", "to": "USDT"};
    } else {
      params = {"to": "IFC", "from": "USDT"};
    }
  ...
  changeConvertCoin(isIfc, val) async {
    var params;
    if (isIfc) {
      params = {"from": "IFC", "to": "USDT", "volume": val};
    } else {
      params = {"to": "IFC", "from": "USDT", "volume": val};
    }
  ...
  _postWalletTransfer(isIfc) {
    ...
    if (_phoneController.text.trim() == null ||
              ...
              child: new Row(
                ...
                children: <Widget>[
                  Flexible(
                    flex: 3,
                    child: new Text(
                      isIfc ? "   可兑换USDT" : "   可兑换IFC",
                      style: TextStyle(
                          color: Color.fromRGBO(51, 51, 51, 1), fontSize: 14.0),
                    ),
                  ),
                  Flexible(flex: 1, child: new Text(volume + "      ")),
                ],
              ),
            ),
          ],
